...
df = pd.read_csv('//wsl.localhost/Ubuntu/home/walkerlaw/reu/matrixoptim/data_model1.csv', index_col=0, header=0, delimiter=',')
...
# columns are partials w.r.t. Eb, Ef, thetae, Pp
...
for i in range(len(df)):
     ...
     A = [[Eb[i]*(2*mp-4*Ef[i]*(math.sin(thetae[i]/2)**2))/(2*math.sqrt(W2)),                       #dW/dEb
          ...
          (-4*Eb[i]*Ef[i]*math.sin(thetae[i]/2)*math.cos(thetae[i]/2))/(2*math.sqrt(W2)),           #dW/dthetae
          0],                                                                                       #dW/dPp
          
          [Eb[i]*1,                                                                                 #dEm/dEb
           Ef[i]*-1,                                                                                #dEm/dEf
           0,                                                                                       #dEm/dthetae
           Pp[i]*(Pp[i])/(math.sqrt((Pp[i]**2+mp**2)**(1/2)))],                                     #dEm/dPp
     
          [0,                                                                                       #dPmx/dEb
           Ef[i]*-math.sin(thetae[i]),                                                              #dPmx/dEf
           -Ef[i]*math.cos(thetae[i]),                                                              #dPmx/dthetae
           Pp[i]*-math.sin(thetap[i])],                                                             #dPmx/dPp

          [Eb[i]*1,                                                                                 #dPmz/dEb
           Ef[i]*-math.cos(thetae[i]),                                                              #dPmz/dEf
           Ef[i]*math.sin(thetae[i]),                                                               #dPmz/dthetae
           Pp[i]*-math.cos(thetap[i])]]                                                             #dPmz/dPp
     M = np.vstack((M, A))
