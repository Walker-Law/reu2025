...
phie   = np.deg2rad(df.loc[:, 'phie'].to_numpy())
phip   = np.deg2rad(df.loc[:, 'phip'].to_numpy())
Pp      = df.loc[:, 'Pp'].to_numpy()


M = np.empty((0, 6))

# columns are partials w.r.t. Ef, thetae, thetap, phie, phip, Pp
...
for i in range(len(df)):
     W2 = mp**2 + 2*mp*(Eb[i]-Ef[i]) - 4*Eb[i]*Ef[i]*(math.sin(thetae[i]/2)**2)           
                                                                                          #abs /#relative values (eventually)
     A = [[Ef[i]*(-2*mp-4*Eb[i]*(math.sin(thetae[i]/2)**2))/(2*math.sqrt(W2)),                  #dW/dEf
...
#uncertainties for the spectrometer offsets (Ef, thetae, thetap, phie, phip, Pp)
sigx = np.array([0.0001, 0.0005, 0.0005, 0.0005, 0.0005, 0.0001])  # example uncertainties
...
print(f'Initial chi²: {initchi2}, so initial chi2/dof value is {initchi2 / ((5-1)*(len(df)-1))} for 5 kinematic settings and {len(df)} runs (since (5-1)*({len(df)}-1)={4*(len(df)-1)} degrees of freedom).') 
...
print(f'Final chi²: {finalchi2}, so final chi²/dof value is {finalchi2 / ((5-1)*(len(df)-1))} for 5 kinematic settings and {len(df)} runs (since (5-1)*({len(df)}-1)={4*(len(df)-1)} degrees of freedom).') 
print('Chi²/dof values less than 1.25 are generally considered acceptable.')
print(f'Spectrometer offsets: \ndelta E_f / E_f (factor - 1): {x[0]} ± {math.sqrt(Cov[0,0])} \ndelta theta_e (in radians): {x[1]} ± {math.sqrt(Cov[1,1])} \ndelta theta_p (factor - 1): {x[2]} ± {math.sqrt(Cov[2,2])} \ndelta phi_e (in radians): {x[3]} ± {math.sqrt(Cov[3,3])} \ndelta phi_p (in radians): {x[4]} ± {math.sqrt(Cov[4,4])} \ndelta P_p / P_p (factor - 1): {x[5]} ± {math.sqrt(Cov[5,5])}')
